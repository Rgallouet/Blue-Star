...
public class PlayerController : MonoBehaviour {
    ...
    //Position discrete tracker
    public int rigidBody_x;
    public int rigidBody_y;
	...
	void FixedUpdate () {
        ...
        if (leftJoystickInput != Vector3.zero)
        {
            ...
            // Check if the player stepped into a new tile

            int new_x = Convert.ToInt32(Math.Floor( rigidBody.transform.position.x + 2.0f * rigidBody.transform.position.y));
            int new_y = Convert.ToInt32(Math.Floor(-rigidBody.transform.position.x + 2.0f * rigidBody.transform.position.y));


            if (new_x != rigidBody_x | new_y != rigidBody_y)
            {
                rigidBody_x = new_x;
                rigidBody_y = new_y;
